...
const EmotionalJourneyMapper = ({ eventData, onSave, initialData, viewMode = false }) => {
  ...
  useEffect(() => {
    ...
    if (initialData) {
      setJourneyMap(initialData.journeyMap || initialData);
      
      // Initially expand the first section
      if (initialData.journeyMap?.journey || initialData.journey) {
        const journey = initialData.journeyMap?.journey || initialData.journey;
    ...
    else if (!viewMode && hasRequiredEventData()) {
      generateJourneyMap();
    }
  ...
  const generateJourneyMap = async () => {
    if (!hasRequiredEventData()) {
      setError('Please fill in event type, capacity, description, and times to generate a journey map.');
      return;
    }
    ...
    try {
      ...
      const response = await fetch('/api/emotional-journey/generate', {
        ...
        body: JSON.stringify({
          ...
          eventGoals: eventData.description,
          // These are optional
      ...
      // Initially expand the first section
      if (data.journeyMap?.journey) {
        if (data.journeyMap.journey.length > 0) {
          setExpandedSections({ 0: true });
        }
      }
      ...
      setError(`Unable to generate emotional journey map: ${err.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  // Get journey data safely, handling different response structures
  const getJourneyData = () => {
    if (!journeyMap) return null;
    
    // Handle different response structures
    const journey = journeyMap.journey || [];
    const metadata = journeyMap.metadata || {};
  ...
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden">
      ...
      <div className="p-6">
        {/* Desired Emotions Input - Only show if not in view mode */}
        {!viewMode && (
          <div className="mb-6">
        ...
        {loading ? (
          ...
          <div className="text-center py-8">
            <p className="text-gray-600 mb-4">
              {hasRequiredEventData() 
                ...
                : "Complete your event details (type, capacity, description, and times) to generate an emotional journey map."}
            </p>
            <button 
          ...
          <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      ...
                      <p className="font-medium">{getJourneyData().metadata?.duration || calculateEventDuration()} hours</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      ...
                      <p className="font-medium">{getJourneyData().metadata?.audienceSize || eventData.capacity} attendees</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {/* Journey Segments */}
            {getJourneyData() && getJourneyData().journey && getJourneyData().journey.length > 0 && (
              <div className="border-t border-gray-200 pt-4">
                <h3 className="text-lg font-medium text-gray-900 mb-4">Emotional Journey Segments</h3>
                
                <div className="space-y-3">
                  {getJourneyData().journey.map((segment, index) => (
                    <div 
                      key={index} 
